...
void ofxKinectDepthUtils::updateDepthImage(ofxKinect *kinect)
{
    int index;
    ofVec3f coordinate;
    int totalPixels = depthImageXYZ.getWidth() * (depthImageXYZ.getHeight() / 3) * 3;
...
//        depthImagePixels[i] = 0;
    ...
    for(int y = 0; y < depthImageXYZ.getHeight() / 3; y++)
    {
        for (int x = 0; x < depthImageXYZ.getWidth(); x++)
        //for (int x = depthImageXYZ.width; x > 0; x--)
        {
            //kinectManager->kinect.getWorldCoordinateAt(x, y);
            index = (y * (depthImageXYZ.getWidth() * 3) + (x * 3));
...
//                coordinate = kinect->getWorldCoordinateAt(x * step, y * step);
           ...
           //cout<< depthImagePixels[index] <<"end"<<endl;
           //  std::cout << kinect->getWorldCoordinateAt(coordX, coordY) << "Inside" << std::endl;
...
//        if (ofRandomuf() < 0.4)depthImagePixels[i] = 0;
    ...
    depthImageXYZ.setFromPixels(depthImagePixels, depthImageXYZ.getWidth(), depthImageXYZ.getHeight(), OF_IMAGE_COLOR);
